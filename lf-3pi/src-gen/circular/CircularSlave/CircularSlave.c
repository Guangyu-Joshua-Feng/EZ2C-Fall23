...
// file://home/ee149/EZ2C-Fall23/lf-3pi/src/circular/CircularSlave.lf
#define LOG_LEVEL 2
#define TARGET_FILES_DIRECTORY "/home/ee149/EZ2C-Fall23/lf-3pi/src-gen/circular/CircularSlave"
...
void _lf_create_environments() {
    environment_init(&envs[circularslave_main],circularslave_main,_lf_number_of_workers,0,1,0,0,0,0,0,NULL);
}
...
void _lf_initialize_trigger_objects() {
    ...
    _circularslave_main_main_self_t* circularslave_main_self[1];
    SUPPRESS_UNUSED_WARNING(circularslave_main_self);
    // ***** Start initializing CircularSlave of class CircularSlave
    circularslave_main_self[0] = new__circularslave_main();
    circularslave_main_self[0]->base.environment = &envs[circularslave_main];
    bank_index = 0; SUPPRESS_UNUSED_WARNING(bank_index);
    envs[circularslave_main].startup_reactions[startup_reaction_count[circularslave_main]++] = &circularslave_main_self[0]->_lf__reaction_0;
    SUPPRESS_UNUSED_WARNING(_lf_watchdog_count);
    
    circularslave_main_self[0]->_lf__reaction_0.deadline = NEVER;
    //***** End initializing CircularSlave
    // **** Start deferred initialize for CircularSlave
    {
    
        // Total number of outputs (single ports and multiport channels)
        // produced by reaction_1 of CircularSlave.
        circularslave_main_self[0]->_lf__reaction_0.num_outputs = 0;
        {
            int count = 0; SUPPRESS_UNUSED_WARNING(count);
        }
        
        // ** End initialization for reaction 0 of CircularSlave
    
    }
    // **** End of deferred initialize for CircularSlave
    // **** Start non-nested deferred initialize for CircularSlave
    {
    
    
    
    }
    // **** End of non-nested deferred initialize for CircularSlave
    // Connect inputs and outputs for reactor CircularSlave.

    
    // Set reaction priorities for ReactorInstance CircularSlave
    {
        circularslave_main_self[0]->_lf__reaction_0.chain_id = 1;
        // index is the OR of level 0 and 
        // deadline 9223372036854775807 shifted left 16 bits.
        circularslave_main_self[0]->_lf__reaction_0.index = 0xffffffffffff0000LL;
    }
